...
class AmadeusService:
    ...
    async def search_flights(self, 
        ...
        params = {
            ...
            "adults": adults,
            "max": 250  # Fetch maximum allowed offers per Amadeus spec (default max limit)
        }
        ...
        try:
            ...
            if e.response.status_code == 401:
                logger.error("Amadeus Token Unauthorized")
                raise HTTPException(status_code=401, detail="Authentication error internally.")
            if e.response.status_code == 429:
                logger.warning("Amadeus Rate Limit Exceeded")
                raise HTTPException(status_code=429, detail="Rate limit exceeded downstream.")
            if e.response.status_code == 401: # This condition is duplicated, keeping as per instruction
    ...
    async def get_airport_destinations(self, hub: str, max_results: int = 15) -> List[str]:
        """
        Queries the Amadeus Airport Routes API to extract direct destinations from a hub organically.
        """
        token = await self._get_token()
        headers = {
            "Authorization": f"Bearer {token}",
        ...
        params = {
            "departureAirportCode": hub,
            "max": max_results
        }
        
        search_url = f"{self.base_url}/v1/airport/direct-destinations"
        ...
        try:
            async with httpx.AsyncClient(timeout=10.0) as client:
                ...
                for item in data.get("data", []):
                    dest_code = item.get("iataCode") or item.get("destinationLocationCode") 
                    if dest_code:
            ...
            if e.response.status_code == 429:
                logger.warning(f"Amadeus Rate Limit Exceeded fetching destinations for {hub}.")
            else:
                logger.error(f"Failed to fetch destinations for {hub}: {e.response.text}")
            return []
        except Exception as e:
            logger.error(f"Failed to fetch destinations bounds internally for {hub}: {e}")
            return []
    ...
    def _normalize_offers(self, amadeus_response: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        for item in amadeus_response.get("data", []):
            try:
                ...
                # Airline Metadata
                number_of_bookable_seats = item.get("numberOfBookableSeats")
                
                # Cabin and Fare Basis
                cabin_class = None
                fare_basis = None
