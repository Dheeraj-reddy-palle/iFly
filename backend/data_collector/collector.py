...
from data_collector.providers import ProviderManager
...
def _upsert_offers(db: Session, offers: List[Dict[str, Any]]) -> dict:
    """Batches insertion using ON CONFLICT DO NOTHING."""
    if not offers:
...
async def main():
    logger.info("Initializing iFly Collector Pipeline (Multi-Provider)...")
    
    db = SessionLocal()
    
    # Initialize ProviderManager with failover: Amadeus → Synthetic
    ...
    if not total_routes or total_routes == 0:
        logger.warning("No active routes found in routes_master!")
        db.close()
        return
    ...
    # Even if Amadeus quota is exhausted, synthetic provider can fill in
    if dynamic_batch_size < 1:
        logger.info("Amadeus quota low — synthetic provider will handle this run.")
        dynamic_batch_size = ROUTE_BATCH_SIZE_MAX  # Let synthetic handle full batch
    ...
    if not active_routes:
        logger.warning("Batch empty! Resetting offset to 0.")
        current_offset = 0
        active_routes = db.execute(active_routes_query, {"limit": effective_batch_size, "offset": current_offset}).fetchall()
        if not active_routes:
    ...
    if max_api_calls_env:
        ...
        if expected_calls > max_api_calls:
            ...
            logger.warning(f"Reducing batch from {actual_batch_size} to {allowed} (MAX_API_CALLS_PER_RUN={max_api_calls}).")
    ...
    logger.info(f"Targeting {total_combinations} route/date combinations using {actual_batch_size} routes.")
    ...
    for row in active_routes:
        ...
        for offset in DATE_OFFSETS:
            ...
            if stats["total_inserted"] >= MAX_OFFERS_PER_RUN:
                logger.warning(f"SAFETY CAP HIT: {MAX_OFFERS_PER_RUN}. Terminating gracefully.")
                break
            ...
            # Use ProviderManager (Amadeus → Synthetic failover)
            offers = await provider_mgr.fetch_offers(origin, destination, target_date)
